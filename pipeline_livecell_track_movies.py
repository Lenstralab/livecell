...
from tiffwrite import IJTiffWriterMulti, IJTiffWriter, tiffwrite
...
def calculate_general_parameters(params):
    ...
    for i in range(0, params['lenExpList']):
...
def parseParams(params):
    ...
    """
    required  = ['pathIn', 'outputfolder']
    optionals = {'CellMask': True, 'OptimizeThreshold': True, 'RunLocalize': True, 'WriteFiles': True,
                 'WriteMovies': True, 'CalculateBackground': True, 'ComputeCorrelFunc': True, 'ChannelsToAnalyze': None,
                 'swapColors': False, 'fillIn': False, 'fillInRadius': 5, 'fillInColors': 'red', 'sideViews': None,
                 'frames': None, 'RegisterChannels': False, 'findCellsCCdist': 25}
    locParams = {'thresholdSD_Red': 5, 'thresholdSD_Green': 6, 'diffThresholdSD': 2, 'writeLocTifFiles': True,
                 'psfPx': 2, 'maxDist': 3, 'minSeparation': 0.1, 'dist': 10, 'winSize': 5, 'TSregfile': [],
                 'TSregRadius': 10}
    ...
    if params['RunLocalize']:
        for k, v in locParams.items():
            if not k in params:
                print(misc.color('Parameter {} missing in parameter file, adding with value: {}'.format(k, v), 'r'))
                params[k] = v
...
def maxProjection(params):
    ...
    with imr(params['pathIn']) as im, \
            ...
            IJTiffWriterMulti((params['domax']*params['maxFile'], params['dosum']*params['sumFile']),
        ...
        for c in params['ChannelsToAnalyze']:
            ...
            for t in range(params['frames']):
                imMax = np.nanmax(im[c, :params['zSlices'], t], 3).squeeze()
                imSum += imMax
                if params['domax']:
...
def cellMask(params):
    with imr(params['sumFile']) as im:
        if len(params['channels'])>1:
            jm = im(params['channels'][1], 0, 0) # uses green image
        else:
            jm = im(params['channels'][0], 0, 0)
        cells = findcells(jm, ccdist=params['findCellsCCdist'])[0]
        if params['RegisterChannels']:
            used_mc = params['masterChannel']==params['channels'][1]
            if used_mc:
                mcells = cells.copy()
                scells = transforms.transform(cells, params['transform'], False)
            else:
                mcells = transforms.transform(cells, params['transform'], True)
                scells = cells.copy()

            #save a mask for each channel, they might be different because of the transforming
            stack = np.zeros(im.shape[:3])
            for c in range(im.shape[2]):
                if params['masterChannel']==c:
                    stack[:,:,c] = mcells
                else:
                    stack[:,:,c] = scells
            tiffwrite(params['expPathOut'] + "_sum_cells_mask.tif", stack.astype('uint16'), 'XYC')
        else:
            tifffile.imsave(params['expPathOut'] + "_sum_cells_mask.tif", cells.astype('uint16'), imagej=True)
    ...
    with imr(imagefile) as imtmp:
        ...
        for channel in params['ChannelsToAnalyze']:
            ...
            for j in trange(nbIm, desc='Optimizing threshold'):
                ...
                if params['TSregfile'] != []:
                    imTS = np.zeros(size)
                    for TS in range(0, len(listCellnr)):
                ...
                for i in range(len(threshvals)):
                    celllisttmp = []
                    imBinary = (imBpass > threshvals[i] * np.var(imBpass) ** .5) * 1.  # Binary image for high threshold
                    ...
                    spotslistunique[i] = spotslistunique[i] + len(misc.uniqueFromList(celllisttmp))
                    try:
            ...
            for x in range(len(celllist)):
                cellswithspots.append(len(misc.uniqueFromList(celllist[x])))
...
def writeFiles(params):
    ...
    if not os.path.exists(listFilesOut):
        open(listFilesOut, "w").write("# coding: utf-8" + "\n" + "listFiles=[" + "\n")
        if TSregfile == []:
        ...
        with open(listFilesOut, "a") as f:
            for i in range(0, len(listCellnr)):
...
def writeMovies(params):
    ...
    daf.showTracking(params['loadedData'], channels, params['expPathOut'], params['sideViews'],
                          params['zSlices'], (0, params['frames']), rgb=True, transform=transform)
...
def computeCorrelFunc(params):
    ...
    ss = fluctuationAnalysis.compAvgCF([(lambda dA: fluctuationAnalysis.mcSig(dA.t, np.c_[dA.r, dA.g].T,
                                      mask=(lambda ti: (dA.frameWindow[0] <= ti) * (ti <= (dA.frameWindow[1])) * 1.)(
                                          np.r_[:dA.t.shape[0]])))(loadedData[i]) for i in range(len(loadedData))], ctr=0,
                   methAvg='arithm', nBs=1000, verb=1)
...
def pipeline(params):
    ...
    for exp in range(0, params['lenExpList']):
...
if __name__ == '__main__':
    if len(sys.argv)<2:
        if os.path.exists('pipeline_livecell_track_movies_parameters.yml'):
            parameter_file = 'pipeline_livecell_track_movies_parameters.yml'
        elif os.path.exists('pipeline_livecell_track_movies_parameters.py'):
            parameter_file = 'pipeline_livecell_track_movies_parameters.py'
        else:
