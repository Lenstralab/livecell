...
def findcells(im, imnuc=None, cellcolormask=None, ccdist=None, threshold=None, thresholdnuc=None, thres=1, thresnuc=1, smooth=2.5, smoothnuc=2.5, minfeatsize=5, minfeatsizenuc=5, dilate=5, dilatenuc=5, removeborders=True):
    ...
    if imnuc is None: #only one channel
        ...
        for i in tqdm(lbl, disable=len(lbl)<25): #threshold each cell to find its nucleus
            cellmask = cells==i
            cmLA = cellmask*LA
            cell = cmLA.flatten()
            cell = cell[cell>0]
        ...
        nuclei, _ = findcells(imnuc, cellcolormask=cellcolormask, ccdist=ccdist, threshold=thresholdnuc, thres=thresnuc, smooth=smoothnuc, minfeatsize=minfeatsizenuc, dilate=dilatenuc, removeborders=False)
        mask, LA = make_mask(im, threshold, thres, smooth, minfeatsize, dilate)
